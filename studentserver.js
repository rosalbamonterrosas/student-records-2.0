...
const addStudent = (recordId, req, res) => {
  MongoClient.connect(uri, { useUnifiedTopology: true }, function(err, client) {
    ...
    dbo.collection("students").findOne(query, function(err, mongoRes) {
      if (err) {
        return res.status(500).send(
          {"message":"error - internal server error: cannot fetch"}
          );
      }
      if (mongoRes == null) {  // no duplicate found
...
const insertStudentObj = (recordId, obj, dbo, client, res) => {
  dbo.collection("students").insertOne(obj, function(err, mongoRes) {
    if (err) {
      return res.status(500).send(
        {"message":"error - internal server error: cannot insert"}
        );
    }
    let rspObj = {};
    rspObj._id = recordId;
...
const findStudent = (recordId, res) => {
  MongoClient.connect(uri, { useUnifiedTopology: true }, function(err, client) {
    ...
    dbo.collection("students").findOne(query, function(err, mongoRes) {
      ...
      var rspObj = {};
      client.close();
      if (mongoRes == null) {  // no match found
        rspObj._id = recordId;
        rspObj.status = 404;
...
const searchStudents = (firstName="", lastName="", res) => {
  MongoClient.connect(uri, { useUnifiedTopology: true }, function(err, client) {
    ...
    dbo.collection("students").find(query).toArray(function(err, mongoRes) {
      if (err) {
        return res.status(500).send(
      ...
      client.close();
      if (mongoRes.length == 0) {  // no matches found
...
const updateStudent = (recordId, obj, res) => {
  MongoClient.connect(uri, { useUnifiedTopology: true }, function(err, client) {
    ...
    dbo.collection("students").updateOne(query, newvals, function(err, mongoRes) {
      if (err) {
        return res.status(500).send(
      ...
      var rspObj = {};
      rspObj._id = recordId;
      if (mongoRes.modifiedCount == 0) {  // no match found
        rspObj.status = 404;
        rspObj.message = 'error - resource not found';
      } else {  // match found
        rspObj.status = 201;
...
const deleteStudent = (recordId, res) => {
  MongoClient.connect(uri, { useUnifiedTopology: true }, function(err, client) {
    ...
    dbo.collection("students").deleteOne(query,function(err, mongoRes) {
      if (err) {
        return res.status(500).send(
          {"message":"error - internal server error: cannot delete"}
          );
      }
      var rspObj = {};
      rspObj._id = recordId;
      if (mongoRes.deletedCount == 0) {  // no match found
        rspObj.status = 404;
        rspObj.message = 'error - resource not found';
      } else {  // match found
        rspObj.status = 200;
